...
class transformationss:
    ...
    def upsert(self,df,key_cols,table):
        ...
        dlt_obj=DeltaTable.forName("pyspark.silver.{table}")
        dlt_obj.alias("trg").merge(df.alias("src"),merge_condition).whenMatchedUpdateAll(condition='src.{cdc}>=trg.{cdc}').whenNotMatchedInsertAll().execute()
        return 1